...
function Invoices({ invoices, setInvoices, companyInfo, user }) {
  ...
  const handleEditedInvoiceChange = (field, value) => {
    setEditedInvoice((current) => ({ ...current, [field]: value }));
  };
        ...
        <table className="w-full">
          ...
          <tbody>
            ...
            {filteredInvoices.map((invoice) => {
              ...
              const isEditing = editingInvoiceId === invoice.id && isAdmin;
              ...
              return (
                <tr key={invoice.id} className="border-b dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700 transition">
                  ...
                  <td className="p-4">
                    {isEditing ? (
                      <input
                        ...
                        onChange={(e) => handleEditedInvoiceChange("clientName", e.target.value)}
                        className="w-full p-2 rounded-xl border dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                      />
                    ) : (
                      invoice.clientName
                    )}
                  ...
                  <td className="p-4">
                    {isEditing ? (
                      <select
                        value={editedInvoice.status || "Draft"}
                        onChange={(e) => handleEditedInvoiceChange("status", e.target.value)}
                        className="p-2 rounded-xl border dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                      >
                        <option value="Draft">Draft</option>
                        <option value="Pending">Pending</option>
                        <option value="Paid">Paid</option>
                  ...
                  <td className="p-4">
                    {isEditing ? (
                      <input
                        ...
                        onChange={(e) => handleEditedInvoiceChange("dueDate", e.target.value)}
                        className="p-2 rounded-xl border dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                      />
                    ) : (
                      invoice.dueDate
